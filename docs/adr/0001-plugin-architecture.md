...
registry in `src/plugins/`, _not_ runtime-loaded. `code`, `links`, `tags`, `todos`, `daily`, and
...
(show/hide, fade, sort, breadcrumb) need this field?_ Yes → it's a core primitive on `Node`. No →
...
- Add plugin-owned fields to the `Node` schema (migrations, the no-schema-defaults problem, sync
  ...
  "protected node" is a _composed predicate_, not a `protected:` column.
